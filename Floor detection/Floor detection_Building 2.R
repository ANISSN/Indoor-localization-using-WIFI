...
wifi_training_nodup_b0 <- wifi_training_nodup %>% 
  filter(BUILDINGID == 2) %>% 
  mutate(FLOOR = factor(FLOOR, levels = c(0,1,2,3,4)))
...
testing_data <- wifi_validation %>% filter(BUILDINGID == 2) %>% mutate(FLOOR = factor(FLOOR, levels = c(0,1,2,3,4))) %>% 
